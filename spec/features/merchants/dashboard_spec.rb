...
RSpec.describe 'Merchant Dashboard Show Page' do
  describe 'Merchant Dashboard Show Page' do
    ...
    let!(:invoice4) { create :invoice, { customer_id: customer3.id} }
    let!(:invoice5) { create :invoice, { customer_id: customer4.id} }
    let!(:invoice6) { create :invoice, { customer_id: customer5.id} }
    ...
    let!(:inv_item7) { create :invoice_item, { item_id: item3.id, invoice_id: invoice6.id } }
    let!(:inv_item8) { create :invoice_item, { item_id: item3.id, invoice_id: invoice7.id } }
    let!(:inv_item9) { create :invoice_item, { item_id: item3.id, invoice_id: invoice8.id } }
    ...
    describe 'statistics - favorite customers' do
      it 'shows names of the top five customers with successful transactions' do
        ...
        expect(fav2).to appear_before(fav3)
        expect(page).to have_content(fav2)
        expect(page).to have_content(fav3)
        expect(page).to have_content(fav4)
        expect(page).to have_content(fav5)
        expect(page).to have_content(fav6)
      end
    end
  end
end
